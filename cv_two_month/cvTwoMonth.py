...
# Local imports
from json_process_cv_two_month import json_data_extract
...
def pdf_gen(json_data):
    ...
    data = json_data
    datarow, month_list = json_data_extract(data)
    single_level_flag = False
    if len(datarow[0]) == 12:
        single_level_flag = True  # Based on the number of levels, two different types of tables are generated
    ...
    if single_level_flag:
        cell_x = (right - left) / 12
        col_widths = [cell_x+80, cell_x-10, cell_x-10, cell_x-10, cell_x-10, cell_x-10, cell_x-10, cell_x-10,
                      cell_x, cell_x, cell_x, cell_x-10]
    else:
        cell_x = (right - left) / 11
        col_widths = [cell_x+80, cell_x-10, cell_x-10, cell_x-10, cell_x-10, cell_x-10, cell_x-10, cell_x-10,
                      cell_x, cell_x, cell_x-10]

    multi_line_col = [col_widths[0], col_widths[1], col_widths[4]]
    ...
    pdf.drawString((right - left) / 2 - 47, top - 20, "均值/CV环比报告")
    ...
    try:
        ...
        month2 = ''
    if single_level_flag:
        headers = [['测定项目', '', month1, '', '', month2, '', '', '均值', 'CV','', ''],
                   ['', '水平', '均值', 'CV', '数据点', '均值', 'CV', '数据点', '比较情况', '比较情况', 'CVR', '质量目标']]
    else:
        headers = [['测定项目', '', month1, '', '', month2, '', '', '均值', 'CV', ''],
                   ['', '水平', '均值', 'CV', '数据点', '均值', 'CV', '数据点', '比较情况', '比较情况', '质量目标']]
    ...
    while True:
        if fst_run:
            ...
            table_data = _wrappable(table_data, 4)
            ...
            _color_changer(table, table_data)
...
def _data_counter(data):
    ...
    """
    tests = len(data["projectMonthDataList"])
    count = 0
    for i in range(tests):
        lev = len(data["projectMonthDataList"][i]["monthDataList"])
        count = count + lev
    return count


def _color_changer(table, table_data):
    """
    ...
    for row in range(0, len(table_data)):
        try:
            cv_perc = float(table_data[row][cv_col][:-1])
            mean_perc = float(table_data[row][mean_col][:-1])
        except:
            cv_perc = ''
            mean_perc = ''

        if type(cv_perc) == float:
            if abs(cv_perc) > 30:
